...
def run_generate(
    ...
    if essence:
        ...
        essence_section = (
            ...
            "**YAML 작성 규칙 (위반 시 파싱 실패 → 슬랙 카드에 빈 chip 노출)**:\n"
            "- 한 줄짜리 값: 그대로 적되 콜론(`:`) 뒤 공백 1칸. 따옴표 안 씀.\n"
            "- **여러 문장/콤마/따옴표가 섞이는 값은 반드시 literal block(`|`) 사용**.\n"
            "  나쁜 예: `falsifiable_by: \"A\", \"B\", \"C\"` ← YAML이 `expected <block end>, but found ','`로 깨짐\n"
            "  좋은 예:\n"
            "  ```yaml\n"
            "  falsifiable_by: |\n"
            "    \"A 시나리오\"가 재현되면 위반.\n"
            ...
            "    \"C 시나리오\"도 위반.\n"
            "  ```\n"
            "- 콜론, 화살표(→), 한자/이모지 포함된 값도 literal block 권장.\n"
            ...
            "    rationale: |\n"
            "      <왜 본질인지 1-2줄, 사용자 요청 어느 부분에서 도출했는지>\n"
            "    falsifiable_by: |\n"
            "      <어떻게 깨졌다고 입증할 수 있나, 측정/관찰 방법 — 여러 시나리오면 줄바꿈으로>\n"
